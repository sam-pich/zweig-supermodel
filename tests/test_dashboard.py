...
from zweig_supermodel.dashboard import (
    DashboardScenario,
    build_dashboard_payload,
    build_dashboard_payload_for_scenarios,
)
...
def test_dashboard_payload_contains_run_series_and_components(monthly_series) -> None:
    ...
    assert payload["runs"][0]["id"] == "book__sp500__super_sp500"
    assert payload["runs"][0]["scenarioName"] == "Book Rules"
    assert "drawdown" in payload["runs"][0]["monthly"][0]
    assert payload["runs"][0]["signals"][0]["signal"] == "buy"
    assert "book__super_sp500" in payload["components"]


def test_dashboard_payload_can_combine_scenarios(monthly_series) -> None:
    close = monthly_series([100, 110, 105])
    model = pd.DataFrame(
        {
            "points": [2, 6, 8],
            "state": ["sell", "buy", "buy"],
    ...
    payload = build_dashboard_payload_for_scenarios(
        [
            DashboardScenario(
                "book",
                "Book Rules",
    ...
    assert [scenario["id"] for scenario in payload["scenarios"]] == ["book", "aggressive"]
    assert [run["scenarioId"] for run in payload["runs"]] == ["book", "aggressive"]
